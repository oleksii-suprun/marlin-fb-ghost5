...
// Searches file `filename` and returns 8.3 name into `dosfilename`.
// Returns 1 when file found, 0 - otherwise
...
void mks_wifi_start_file_upload(ESP_PROTOC_FRAME *packet){
   ...
   WRITE(MKS_WIFI_IO4, HIGH); // Stop transmission from ESP
   ...
   thermalManager.task();
 	// Set file name. 3 bytes offset to add path to disk.
   file_name[0]='0';
   file_name[1]=':';
   file_name[2]='/';
   ...
   // Unmount SD from Marlin, mound FATFs
   if(mks_wifi_sd_init()){
   ...
   dma_buff_index=0;
   file_inc_size=0; // Input data counter for writing to file
   file_size_writen = 0; // Counter for data written to file
   file_data_size = 0;
...
// delay(200);
   ...
   while(--dma_timeout > 0){
      ...
      if(buffer_ready > 0){
         ...
         data_size -= 4; // 4 bytes with a segment number and flags
         ...
         if(*(data_packet+7) == 0x80){ // Last package with data
            WRITE(MKS_WIFI_IO4, HIGH); // Stop transmission from the ESP
            DEBUG("Last packet");
   ...
   WRITE(MKS_WIFI_IO4, LOW); // Enable transmission from the ESP
